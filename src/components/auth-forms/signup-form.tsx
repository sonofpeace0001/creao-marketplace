...
export function SignupForm() {
  ...
  if (done) {
    return (
      <p className="text-sm text-muted">
        Check <span className="text-fg">{email}</span> for a confirmation link, then log in.
      </p>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="mb-1.5 block text-sm text-muted">Handle</label>
        <input
          type="text"
          required
          value={handle}
